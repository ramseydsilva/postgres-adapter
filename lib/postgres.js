...
PG.prototype.update = function(modelName, params, callback) {
    var model = this;
    this.all(modelName, params.where, function(err, docs) {
        if (err) callback(err);
        if (!!docs.length) {
            async.each(docs, function(doc, next) {
                model.updateOrCreate(modelName, params.update, next);
            }, function(err, docs) {
...
PG.prototype.defineForeignKey = function(className, key, foreignClassName, cb) {
    // Create foreign key if not exists, TODO: need a case to delete foreign key as well
    var fkName = className + '_' + key + '_fk';
    var that = this;
    var exists = false;
    this.query('SELECT "conname" from pg_constraint WHERE contype = \'f\';', function(err, foreignKeyData) {
        foreignKeyData.forEach(function(fk) {
            if (fk.conname == fkName) {
                exists = true;
            }
        });
        if (!exists) {
            var m = that._models[className];
             m.settings.indexes[fkName] = {
                 kind: 'foreignkey',
                 className: className,
                 key: key,
                 foreignClassName: foreignClassName
             };
             console.log(m.settings.indexes);
        }
        cb(null, Number);
    });
}
...
function getIndexesToAdd(model, actualIndexes) {
    ...
    var sql = [];
    if (indexNames) {
        indexNames.filter(function(indexName) { return indexName.indexOf(model+'_') == '0' }).forEach(function (indexName) {
...
function addIndexToActual(model, indexName) {
    ...
    } else if (kind.toLowerCase() === 'foreignkey') {
        sqlCommand = 'ALTER TABLE "' + i.className + '" ADD CONSTRAINT "' + indexName +'"'
            + ' FOREIGN KEY ("'+ i.key + '") REFERENCES "' + i.foreignClassName + '"("id");'
